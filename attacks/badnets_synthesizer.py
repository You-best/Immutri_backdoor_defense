...
class BadnetsSynthesizer(Synthesizer):
    ...
    def make_pattern(self, pattern_tensor, x_top, y_top):
        ...
        # print("({},{})->({},{})".format(x_top, y_top, x_bot, y_bot))
    ...
    def get_pattern(self):
        if self.params.backdoor_dynamic_position:
            ...
            pattern = transform_to_tensor(image).squeeze()
    ...
    def get_pattern_simgle(self):
        ...
        if resize != 1:
            self.make_pattern(pattern, 3, 23)

        return self.pattern, self.mask
    
    def apply_backdoor_to_a_sample(self, data, label, params):
        pattern, mask = self.get_pattern_simgle()
        backdoor_sample = (1 - mask) * data + mask * pattern
